...
export class ListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        this.formData = this._formBuilder.group({
            ...
            name_th: [''],
            service_or: [''],
            cha_no: this._formBuilder.array([]),
            install_no: [null],
            first_name: [''],
            last_name: [''],
            photo_vehicle_gas: [''],
    ...
    async loadTable(status: number, tableIndex: number): Promise<void> {
        ...
        this[dtOptionsKey] = {
            pagingType: 'full_numbers', pageLength: 10, displayStart: this.start, order: [],
            ...
            ajax: (dataTablesParameters: any, callback) => {
                ...
                dataTablesParameters.data_gas = status;
    ...
    // updateStatus(): void {
    //     this._Service.updateStatus(this.formData.value, this.formData.get('id').value).subscribe({
    //         next: () => { this.closeDialog(); this.rerender(); },
    //         error: (err) => { console.error('Update failed', err); this.showFlashMessage('error'); }
    //     });
    // }
    ...
    update(): void {
        this._US.confirmAction('แก้ไขรายการ', 'คุณต้องการแก้ไขรายการใช่หรือไม่', this._fuseConfirm,
            () => {
                this._Service.updateStatus(this.formatForm(), this.formData.get('id').value).subscribe({
    ...
    //     });
    //     if (this.files) { formData.append('photo', this.files); }
    //     return formData;
    // }
    private formatForm(): any {
        const formData = new FormData();
    ...
    openDialog(item?: any, event?: Event): void {
        this.chaNoArray.clear();

        if (item.cha_no && Array.isArray(item.cha_no)) {
            item.cha_no.forEach(value => {
                this.chaNoArray.push(this._formBuilder.control(value));
            });
        }

        if (event) { event.stopPropagation(); }

        item ? (this.formData.patchValue({}), this.isEdit = true) : (this.isEdit = false);
    ...
    pdf() {
        ...
        window.open(`${environment.API_URL}/api/report/gas_report?start_date=${formatSDate}&end_date=${formatEDate}`, '_blank');
    }

    get chaNoArray(): FormArray {
        return this.formData.get('cha_no') as FormArray;
    }
    addInstallNo(): void {
        this.chaNoArray.push(this._formBuilder.control(''));
    }

    removeInstallNo(index: number): void {
        this.chaNoArray.removeAt(index);
    }
