...
export class ListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    lengthsummary:number = 0;
    lengthsummaryhead:number = 0;
    serviceTypes: any[];
    GetReport() {
        const formValue = {
            ...this.form.value
        ...
        // formValue.branch_id= this.user?.employees?.branch_id;

        this._Service.getDailyReport_new(formValue).subscribe((resp: any) => {
            this.dataRow = resp.data
            this.lengthsummary = resp.data.summary.length + 8
            this.lengthsummaryhead = resp.data.summary.length
            this.serviceTypes = resp.data.service_types
            this._changeDetectorRef.markForCheck()
        })
    }
    getIncomeByType(summary: any[], typeId: number): number {
        const service = summary.find(s => s.service_type_id === typeId);
        return service ? service.income : 0;
      }
