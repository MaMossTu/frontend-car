...
export class FormDataFrame {
    ...
    createMainForm(): FormGroup {
        return this._formBuilder.group({
            ...
            employee_id:                    [''], // ตัวแปรใหม่
            branch_id:                      [''], // สาขา
            date:                           [new Date().toISOString().split('T')[0]], // วันที่
            result:                         [''], // ผลการตรวจรถ
            remark:                         [''], // หมายเหตุอื่นๆ
            remark_transport:               [''], // หมายเหตุขนสง
            ...
            total_vat:                      ['0.00'], // ยอดสุทธิ (ตัวแปรใหม่)
            total_vatcal:                   ['0.00'], // ยอดสุทธิ (ตัวแปรใหม่)
            total_nonvat:                   ['0.00'], // ยอดสุทธิ (ตัวแปรใหม่)
            total_nonvatcal:                ['0.00'], // ยอดสุทธิ (ตัวแปรใหม่)
            discount:                       ['0.00'], // ส่วนลด (ตัวแปรใหม่)
            discount_before_vat:            ['0.00'], // ส่วนลด (ตัวแปรใหม่)
            discount_after_vat:             ['0.00'], // ส่วนลด (ตัวแปรใหม่)
            installment_date:               [new Date().toISOString().split('T')[0]], // วันที่จ่าย (ตัวแปรใหม่)
            paid_type: [null],
            ...
            type:                           ['', Validators.required], // ประเภท (vendor, personal)
            ...
            // ข้อมูลประกัน
            insurance_names_id:             [''], // บริษัทประกัน
            ...
            now_paid_vat:                   [''], // จำนวนเงินที่จ่ายในปัจจุบัน
    ...
    createCusCarForm(): FormGroup {
        return this._formBuilder.group({
            ...
            insurance_types_id:             ['', Validators.required],
    ...
    createCusDataForm(): FormGroup {
        return this._formBuilder.group({
            ...
            address:                        [''],
            selectedAddressId:              [''],
            address_arr:                    this._formBuilder.array([]), // ที่อยู่ลูกค้า
        });
    }
