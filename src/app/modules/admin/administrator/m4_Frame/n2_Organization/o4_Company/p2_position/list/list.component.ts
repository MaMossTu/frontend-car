...
export class ListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    // public listDataFromService: any[] = [];
    ...
    async ngOnInit(): Promise<void> {
        ...
        // this._Service.getDataFromService().subscribe(resp => this.listDataFromService = resp);

        this.formGroups['form1'] = this._formBuilder.group
            ({ permission_id: [1], menu_id: [1], view: [0], save: [0], edit: [0], delete: [0], });
        this.formGroups['form2'] = this._formBuilder.group
            ({ permission_id: [1], menu_id: [1], view: [0], save: [0], edit: [0], delete: [0], });

        const resp = await lastValueFrom(this._Service.getPermissions('1'));
        this.formGroups['form1'].patchValue({
            ...resp,
        });
    }
    ...
    task: Task = {
        ...
        completed: false,
        subtasks: [
            { name: 'มองเห็นเมนู', completed: false },
            { name: 'บันทึกข้อมูล', completed: false },
            { name: 'แก้ไขข้อมูล', completed: false },
            { name: 'ลบข้อมูล', completed: false }
        ]
    };

    get partiallyComplete(): boolean {
        const { subtasks } = this.task;
        return subtasks.some(t => t.completed) && !subtasks.every(t => t.completed);
    }

    updatebox(formKey: string, completed: boolean, index?: number): void {
        const value = completed ? 1 : 0;
        const formData = this.formGroups[formKey];

        if (index === undefined) {
            this.task.completed = completed;
            this.task.subtasks.forEach((t, i) => {
                t.completed = completed;
                formData.get(this.getFormControlName(i))?.setValue(value);
            });
        } else {
            this.task.subtasks[index].completed = completed;
            formData.get(this.getFormControlName(index))?.setValue(value);
            this.task.completed = this.task.subtasks.every(t => t.completed);
        }
    }

    getFormControlName(index: number): string {
        const controlNames = ['view', 'save', 'edit', 'delete'];
