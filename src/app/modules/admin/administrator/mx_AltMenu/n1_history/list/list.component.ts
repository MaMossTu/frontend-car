...
export class ListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    public pages = { current_page: 1, last_page: 1, per_page: 25, begin: 0 };
    ...
    loadTable(): void {
        ...
        this.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                ...
                dataTablesParameters.start_date = startDate ? new Date(startDate).toISOString().split('T')[0] : null;
                dataTablesParameters.end_date = endDate ? new Date(endDate).toISOString().split('T')[0] : null;

                const order = dataTablesParameters.order.length > 0
                    ? dataTablesParameters.order[0]
                    : { column: 0, dir: 'desc' };
                that._Service.getPage({ ...dataTablesParameters, order: [order] }).subscribe((resp) => {
    ...
    onDateChange(event: any): void {
        const start = event.value?.start ?? this.formData.get('start_date')?.value;
        const end = event.value?.end ?? this.formData.get('end_date')?.value;
